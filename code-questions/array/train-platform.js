/* Given arrival & departure times of all trains that reach a railway station. 
...
of a train and arrival of another train. 
...
module.exports.findPlatform = function (arr, dep) {
    ...
    let maxPlatforms = 1;
    let currentPlatformsNeed = 1;
    dep.sort((a, b) => a - b);

    for (let i = 1, j = 0; j < dep.length && i < arr.length; ) {
        if (arr[i] >= dep[j]) {
            currentPlatformsNeed--;
            j++;
        } else {
            currentPlatformsNeed++;
            i++;
        ...
        if (maxPlatforms < currentPlatformsNeed) maxPlatforms++;
    }
